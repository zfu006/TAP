...
def test_finetune_rgb(net, device, dataloader):
    psnr = 0
    ssim = 0
    num = 0
...
def main():
    ...
    test_dataset = test_rgb_vid_dataset(gt_dir=opt.test_clean_vid_dir[0], n_frames=opt.n_frames, noise_level=opt.noise_level, 
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            if current_step % opt.test_step == 0:
                ...
                net_G.eval()
                psnr, ssim = test_finetune_rgb(net=net_G, device=device, dataloader=test_loader)
